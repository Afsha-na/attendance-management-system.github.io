...
if ($corepage!=='index.php') {
      if ($corepage==$corepage) {
       ...
       try{
        if($corepage=='admin_index'){
          throw new Exception("");
        }
        else{
       //echo "<script>window.location.href = 'index.php?page='.$corepage[0]';</script>";
       }
      }
       catch(Exception $e)
       {
    ...
    <?php 
     ...
     <?php
        ...
        echo // '<td>'.$i.'</td>
          ...
          <td><img src="../teacher/images/'.$result['photo'].'" height="50px"></td>
          
          <td>
            <a class="btn btn-xs btn-warning" href="admin_index.php?page=admin_editstudent&id='.base64_encode($result['id']).'&photo='.base64_encode($result['photo']).'">
              <i class="fa fa-edit"></i></a>

             &nbsp; <a class="btn btn-xs btn-danger" onclick="javascript:confirmationDelete($(this));return false;" href="admin_index.php?page=admin_delete&id='.base64_encode($result['id']).'&photo='.base64_encode($result['photo']).'">
             <i class="fas fa-trash-alt"></i></a></td>';?>
      </tr>  
     <?php $i++;} }?>
